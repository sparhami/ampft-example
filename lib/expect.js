...
function installLengthWrapper(chai, utils) {
  ...
  const lengthCondition = (expected, value) => {
    return value.length == expected
  };
  const overwriteLength = overwriteWithCondition(utils, lengthCondition);
  Assertion.overwriteChainableMethod(
      'length', overwriteLength, inheritChainingBehavior);
  Assertion.overwriteChainableMethod(
      'lengthOf', overwriteLength, inheritChainingBehavior);
}
...
function installAboveWrapper(chai, utils) {
  ...
  const aboveCondition = function(expected, value) {
    if (flag(this, 'doLength')) {
      value = value.length;
    }
    return value > expected;
  };
  const overwriteAbove = overwriteWithCondition(utils, aboveCondition);
  Assertion.overwriteMethod('above', overwriteAbove);
  Assertion.overwriteMethod('gt', overwriteAbove);
}
