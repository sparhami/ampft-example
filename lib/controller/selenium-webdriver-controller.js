...
import {By, Key, WebElement, Condition, until} from 'selenium-webdriver';
import {
  ControllerPromise,
  ElementHandle,
...
//   return new Condition('until element has attribute with value', () =>
//       element.getAttribute(attribute).then(v => v != value));
// }
...
function waitFor(browser, valueFn, condition) {
  return browser.wait(expectCondition(valueFn, condition))
}
...
export class SeleniumWebdriverController {
  ...
  getWaitFn_(valueFn) {
    ...
    return (expected, opt_condition) => {
      opt_condition =
          opt_condition || (value => value == expected);
      return waitFor(this.browser, valueFn, value => opt_condition(value) ? value : null);
    }
  }
  ...
  getElementText(handle, opt_expect) {
    ...
    if (opt_expect) {
      condition = typeof opt_expect == 'string' ?
        until.elementTextContains(handle, opt_expect) :
