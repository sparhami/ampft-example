...
export class PuppeteerController {
  ...
  /**
   * Waits for scrolling to stop on a given Element.
   * 
   * @param {!ElementHandle<!PuppeteerHandle>} handle
   * @param {{
   *   waitTime: number,
   * }} options
   *    waitTime How long to wait for scrolling to stop.
   * @return {!Promise} A Promise that resolves once scrolling has stopped.
   */
  async waitForScrollingToStop(handle, {waitTime = 500} = {}) {
    await this.page_.evaluate((el, waitTime) => {
      let timer;

      return new Promise(resolve => {
        function complete() {
          el.removeEventListener('scroll', waitForComplete)
          resolve();
        }
        ...
        el.addEventListener('scroll', waitForComplete);
        waitForComplete();
      });
    }, handle.getElement(), waitTime);
  }
   ...
   * @return {!Promise}
   */
  async scrollElementLeftBy(handle, delta) {
    return await this.page_.evaluate((el, delta) => {
      // Need to return the result or it is not executed.
      return el.scrollLeft = delta;
    }, handle.getElement(), delta); 
  }
   ...
   */
  async getElementScrollLeft(handle) {
    return await this.page_.evaluate(el => {
   ...
   * @return {!Promise<number>}
   */
  async getElementScrollWidth(handle) {
    return await this.page_.evaluate(el => {
